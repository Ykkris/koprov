...
namespace FRFuel
{
    public class FRFuel : BaseScript
    {
        ...
        protected float fuelRPMImpact = 0.00025f;
    ...
    public Dev.DevMenu menu;
        ...
        public void ConsumeFuel(Vehicle vehicle)
        {
            ...
            )
            {
                ...
                if (vehicle.IsEngineRunning && max - current > 0.5f)
                {
                    Screen.ShowNotification("Veuillez ~r~couper~w~ le moteur pour commencer a faire le plein.");
                }
                else
                {
                    ...
                    if (refuelAllowed)
                    {
                        if (Game.IsControlPressed(0, Control.Jump) && fuel < fuelTankCapacity)
                        {
                            fuel += 0.1f;
                        ...
                        if (Game.IsControlJustReleased(0, Control.Jump))
                        {
                            ...
                            TriggerServerEvent("essence:payer", addedFuelCapacitor);
        ...
        public void ManualRefuel(Ped playerPed)
        {
            if (playerPed.Weapons.Current.Hash == WeaponHash.PetrolCan)
            {
                ...
                )
                {
                    ...
                    if (max - current < 0.5f)
                    {
                        hud.InstructManualRefuel("Le reservoir est ~r~plein~w~");
                    }
                    else
                    {
                        hud.InstructManualRefuel("Faire le plein");
                    }
