...
function ACL:canConnect( netID, playerName )
	...
	if ACL.useIdentBugWorkaround and lastConnectedIdent ~= nil then
		if ids[1] == lastConnectedIdent[1] then
			...
			self.lastError = "Merci de réessayer."
			return false
		end
	end
	...
	if self.maxPing > 0 and png > self.maxPing then
		self.lastError = "Votre ping est trop haut pour ce serveur (" .. png .. "ms / " .. self.maxPing .. "ms)."
		return false
	end
	...
	self.lastError = "Ce serveur est privé. Merci de vous rendre sur koprov.fr pour être whitelisté."
	return false
end
...
function ACL:isBanned( playerID, playerName )
	...
	if string.upper( playerName ) == "SERVER" or string.upper( playerName ) == "USAGE" then
		self.lastError = string.format( "Le nom '%s' est réservé. Merci de le changer.", playerName )
		return true
	end
	...
	for k,v in pairs( self.banlist ) do
		...
		if string.find( playerName, v, 1, true ) ~= nil then
			self.lastError = "Votre nom ou une partie n'est pas autorisé."
			return true
		end
		
		-- Check for banned individuals
		if findi( ids, v ) ~= nil then
			self.lastError = "Vous êtes bannis."
			return true
		end
	end
...
AddEventHandler( "playerConnecting", function( playerName, setCallback )
	...
	if not ACL:canConnect( source, playerName ) then
		print( "Connexion rejeté pour: " .. playerName .. " (raison: " .. ACL.lastError .. ")" )
	...
	if #playersConnecting > 0 then
		...
		if EP ~= nil then
			...
			if ACL:isBannedIP( EP ) then
				DropPlayer( playersConnecting[1], "Vous êtes bannis." )
			end
