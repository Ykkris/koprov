RegisterNetEvent("selveh:SelVehicle")
...
function RentrerVehicule()
	TriggerServerEvent('selveh:CheckForVeh',source)
CloseMenu()
	...
	while true do
		...
		if GetDistanceBetweenCoords(vente_location[1],vente_location[2],vente_location[3],GetEntityCoords(LocalPed())) < 5 and IsPedInAnyVehicle(LocalPed(), true) == false then
			...
			if IsControlJustPressed(1, 86) then				
				TriggerServerEvent('selveh:CheckForSelVeh',source)
			end
		end
	end
end)

AddEventHandler('selveh:SelVehicle', function(vehicle, plate)
	...
	Citizen.CreateThread(function()		
		...
		if DoesEntityExist(caissei) then	
			if plate ~= platecaissei then					
				...
				TriggerServerEvent('selveh:SelVeh', plate)
				TriggerServerEvent("selveh:CheckGarageForVeh")
			end
		else
